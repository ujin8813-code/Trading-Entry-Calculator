...
class StockMetrics:
    ...
    value_pass: bool
    tech_pass: bool
    earnings_trend: Optional[str]  # "up", "down", "flat", None
    earnings_warning: bool  # 2분기 연속 감소 시 True
...
class StockRepository:
    ...
    def fetch_quarterly_earnings(ticker: str) -> list:
        """최근 분기별 영업이익(또는 순이익) 추이 반환"""
        try:
            t = yf.Ticker(ticker)
            # 분기 손익계산서에서 영업이익 추출
            q_financials = t.quarterly_financials
            if q_financials is not None and not q_financials.empty:
                # Operating Income 또는 Net Income 사용
                for key in ["Operating Income", "Net Income", "EBIT"]:
                    if key in q_financials.index:
                        values = q_financials.loc[key].dropna().tolist()
                        # 최신순 -> 시간순으로 정렬 (최대 4분기)
                        return list(reversed(values[:4]))
            # 대안: earnings 속성
            earnings = t.quarterly_earnings
            if earnings is not None and not earnings.empty:
                if "Earnings" in earnings.columns:
                    return earnings["Earnings"].tolist()[-4:]
            return []
        except Exception:
            return []
...
class AnalysisService:
    ...
    def _calc_technical(self, hist: pd.DataFrame) -> tuple:
        ...
        close = hist["Close"]
        ma120 = close.rolling(window=60).mean().iloc[-1] if len(close) >= 60 else None
    ...
    def _calc_earnings_trend(self, ticker: str) -> tuple:
        """최근 2분기 연속 감소 여부 판정"""
        earnings = StockRepository.fetch_quarterly_earnings(ticker)
        if len(earnings) < 2:
        ...
        if recent[-1] < recent[-2]:
            # 3분기 이상 있으면 3분기 연속도 체크
            if len(earnings) >= 3 and earnings[-2] < earnings[-3]:
                return "down", True  # 2분기 연속 감소 -> 경고!
            return "down", False  # 1분기 감소 (아직 경고 아님)
    ...
    def analyze(self, ticker: str) -> StockMetrics:
        ...
        # ── 실적 추세 판정 ──
        earnings_trend, earnings_warning = self._calc_earnings_trend(ticker)
...
class ChartBuilder:
    ...
    def build_candle_chart(ticker: str, name: str) -> go.Figure:
        ...
        fig.add_trace(go.Scatter(
            x=hist.index, y=ma120, name="60일선",
...
def render_stock_card(m: StockMetrics):
    ...
    ">
        ...
        <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
            ...
            <span style="font-size:11px;padding:3px 8px;border-radius:4px;
                background:{'#064e3b' if m.earnings_trend == 'up' else '#7f1d1d' if m.earnings_warning else '#1e293b'};
                color:{'#6ee7b7' if m.earnings_trend == 'up' else '#fca5a5' if m.earnings_warning else '#94a3b8'};">
                {'📈' if m.earnings_trend == 'up' else '📉' if m.earnings_trend == 'down' else '➖'} 실적{'⚠️ 2Q연속↓' if m.earnings_warning else ''}
            </span>
...
def render_portfolio():
    ...
    for r in rows:
        ...
        ">
            <div>
                ...
                <div style="font-size:11px;color:#475569;">
                    &#8361;{r['buy_price']:,.0f} x {r['quantity']} | {r['buy_date']}
                </div>
            </div>
        ...
        """
        st.components.v1.html(pf_html, height=70)

    st.components.v1.html(f"""
    <div style="text-align:right;font-size:16px;font-weight:700;color:#f59e0b;margin-top:8px;font-family:sans-serif;">
        총 평가: &#8361;{total_value:,.0f}
    </div>
    """, height=40)
...
def main():
    ...
    # ── 헤더 ──
    st.title("🎖️ 투자 사령부")
    st.caption("매수 원칙 기반 스캐너 (실적추세 필터 포함) | 5분 자동갱신 | 데이터 15~20분 지연")
    ...
    # ── 푸터 ──
    st.caption("투자 사령부 v2.1 | 실적추세 필터 + 5분 자동갱신 | 투자 판단의 최종 책임은 본인에게 있습니다 | Powered by yfinance & Streamlit")
